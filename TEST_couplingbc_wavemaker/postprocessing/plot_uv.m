...
subplot(6,1,[3 4])
pcolor(xx,yy,v),shading flat
%caxis([-1 1])

h_bar=colorbar('location','EastOutside');
set(get(h_bar,'xlabel'),'string','v (m/s)' )

hold on
sc=5;
sk=10;
quiver(xx(1:sk:end,1:sk:end),yy(1:sk:end,1:sk:end),u(1:sk:end,1:sk:end)*sc,v(1:sk:end,1:sk:end)*sc,0,'Color','w')
...
%print -djpeg case_slope_055_reduced_v.jpg
